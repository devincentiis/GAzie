...
class FatturaImmediata extends Template_con_scheda
{
    ...
    function newPage() {
        ...
        $this->Cell(7, 6, 'MU', 1, 0, 'C', 1);
        $this->Cell(16, 6, 'Quantity', 1, 0, 'C', 1,0,1);
        $this->Cell(20, 6, 'Price', 1, 0, 'C', 1);
        $this->Cell(6, 6, '%Dsc', 1, 0, 'C', 1,0,1);
        $this->Cell(20, 6, 'Amount', 1, 0, 'C',1, 1);
      ...
      foreach ($lines AS $key => $rigo) {
            if (($this->GetY() >= 157 && $this->taxstamp >= 0.01) || $this->GetY() >= 186 ) { // mi serve per poter stampare la casella del bollo
                ...
                case "0":
                    ...
                    if ($rigo['sconto']>0) {
                       $this->Cell(6,5, floatval($rigo['sconto']),1,0,'C', 0, '', 1);
                    } else {
                       $this->Cell(6,5, '',1,0,'C');
                    }
                ...
                case "2":
                    //$this->Cell(25,5,'','L');
                    $this->Cell(154,5,$rigo['descri'],'LR',0,'L',0,'',1);
                    $this->Cell(32,5,'','R',1);
                    break;
                case "3":
                    $this->Cell(25,5,'',1,0,'L');
                    $this->Cell(80,5,$rigo['descri'],'B',0,'L',0,'',1);
                ...
                case "4":
                    $this->Cell(25,5, $rigo['codart'],1,0,'L', 0, '', 1);
                    $this->Cell(129,5, $rigo['descri'].'('.floatval($rigo['provvigione']).'% di '.gaz_format_number($rigo['prelis']).')',1,0,'L',0,'',1);
                    $this->Cell(20,5, gaz_format_number($rigo['importo']),1,0,'R');
                    $this->Cell(12,5, gaz_format_number($rigo['pervat']),1,1,'R');
                    break;
                case "6":
                case "8":
                    $this->writeHtmlCell(186,5,10,$this->GetY(),$rigo['descri'],1,1);
                    break;
                case "11":
                    $this->Cell(25, 5, '', 'L');
                ...
                case "90":
                    ...
                    $this->Cell(20,5, '', 1);
                    $this->Cell(12,5, '', 1, 1);
                    $this->Cell(105,5, $rigo['descri'],1,0,'L',0,'',1);
                    $this->Cell(49,5, '',1);
                    $this->Cell(20,5, gaz_format_number($rigo['importo']),1,0,'R');
                    $this->Cell(12,5, gaz_format_number($rigo['pervat']),1,1,'R');
                    break;
    ...
    function pageFooter()
    {
        if (!empty($this->descriptive_last_row) ) { // aggiungo alla fine un eventuale rigo descrittivo dalla configurazione avanzata azienda
                $this->Cell(186,5,$this->descriptive_last_row,1,1,'L',0,'',1);
		}
        ...
        if (!empty($this->banapp['descri']) && $this->pagame['tippag'] != 'D' && $this->pagame['tippag'] != 'O') {
           $this->Cell(62, 5, 'Support bank','LTR',1,'C',1);
           ...
           $this->Cell(62, 5, 'Credit bank','LTR',1,'C',1);
        ...
        $this->Cell(124,5, 'Expiry Dates and Rate Amount','LTR',1,'C',1);
        $this->Cell(62, 5, 'Shipping','LTR',0,'C',1);
        ...
        $this->Cell(55, 5);
        $this->Cell(40, 5,'Shipping start','LTR',0,'C',1);
        $this->Cell(56, 5,'Recipient signature','LTR',1,'C',1);
        $this->Cell(90, 5,'','L');
        if ($this->tesdoc['id_tes'] > 0) {
