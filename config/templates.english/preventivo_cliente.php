...
class PreventivoCliente extends Template
{
  ...
  public $_tplIdx;
    ...
    function setTesDoc()
    {
        ...
        $this->tipdoc = 'Quote to customer n.'.$this->tesdoc['numdoc'].'/'.$this->tesdoc['seziva'].' del '.$this->giorno.' '.$this->mese.' '.$this->anno;
    }
    ...
    function body()
    {
		...
		foreach ($lines AS $key => $rigo) {
            if ($this->GetY() >= 185) {
                ...
                $this->SetY(225);
                $this->Cell(186,12,'>>> --- CONTINUED ON NEXT PAGE --- >>> ',1,1,'R');
                $this->SetFont('helvetica', '', 9);
                $this->newPage();
    ...
    function pageFooter()
    {
        ...
        $this->Cell(24, 6,'Bank charges',1,0,'C',1);
        $this->Cell(26, 6,'Delivery fee',1,0,'C',1);
        $this->Cell(36, 6,'Taxable total',1,0,'C',1);
        $this->Cell(26, 6,'VAT total.',1,0,'C',1);
        $this->Cell(22, 6,'Weigth (kg)',1,1,'C',1);
        ...
        $this->Cell(62, 6,'Forwarder',1,1,'C',1);
        $this->Cell(186,6,(isset($vettor['descri'])?$vettor['descri']:''),1,1,'L');
        $this->Cell(186,6,'This quote is valid for  '.$this->tesdoc['day_of_validity'].' working days. After this period, prices and conditions of sale',0,1,'L');
        $this->Cell(186,6,'may undergo changes depending on market situations.','B',1,'L');
                ...
                if ($rigo['ext'] == 'pdf') {
                    ...
                    if ($this->numPages >= 1) {
                        for ($i = 1; $i <= $this->numPages; $i++) {
                            ...
                            $this->SetXY(10, 0);
                            $this->Cell(190, 3,$this->intesta1 . ' ' . $this->intesta1bis." - document attached to: " . $this->tipdoc , 1, 0, 'C', 0, '', 1);
                        }
                    }
                    ...
                    if ($w > $h) { //landscape
						...
						$this->print_footer = false;
                        $this->SetXY(10, 0);
                        $this->Cell(280, 3, $this->intesta1 . ' ' . $this->intesta1bis." - document attached to: " . $this->tipdoc, 1, 0, 'C', 0, '', 1);
