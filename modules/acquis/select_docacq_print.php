...
/* TIPI DI DOCUMENTO SELEZIONABILI PER LA RI/STAMPA:
  ...
  1 => "Fattura di acquisto immediata"
  2 => "Fattura di acquisto differita"
  3 => "Fattura di accredito"
 */
...
function getLastDocument($tipo, $sezione, $anno) {
   ...
   global $gTables;
   switch ($tipo) {
	   case 0:  // Tutti gli AF di acquisto
         $where = "(tipdoc LIKE 'AF_' AND YEAR(datemi) = $anno AND template = 'FatturaAcquisto'";
         break;
      case 1:  //Fattura immediata
         $where = "(tipdoc = 'AFA' AND YEAR(datemi) = $anno AND template = 'FatturaAcquisto'";
         break;
      case 2:  //fattura differita
         $where = "tipdoc = 'AFT' AND YEAR(datfat) = $anno AND template = 'FatturaAcquisto'";
         break;
      case 3:  //fattura accredito
         $where = "tipdoc = 'AFC' AND YEAR(datfat) = $anno AND template = 'FatturaAcquisto'";
         break;
...
function checkDocumentExist($tipo, $sezione, $data_inizio, $data_fine, $protocollo_inizio = 0, $protocollo_fine = 999999999, $numero_inizio = 0, $numero_fine = 999999999, $cliente = 0) {
   ...
   switch ($tipo) {
      ...
      case 1:  //fattura acquisto
         $where = "tipdoc = 'AFA' AND template = 'FatturaAcquisto'";
         break;
      case 2:  //fattura immediata differita con ddt
         $where = "tipdoc = 'AFT' AND template = 'FatturaAcquisto'";
         break;
      case 3: //fattura 
         $where = "tipdoc = 'AFC' AND template = 'FatturaAcquisto'";
         break;      
   }
   ...
   $rs_existdoc = gaz_dbi_dyn_query("*", $gTables['tesdoc'], $where, "$date_name DESC", 0, 1);
...
if (isset($_POST['Print'])) {
   ...
   if (empty($msg)) {
      ...
      if (!checkDocumentExist($form['tipdoc'], $form['seziva'], $datini, $datfin, $form['proini'], $form['profin'])) {
         $msg .="19+"; // "Non ci sono documenti stampabili negli intervalli selezionati !",
   ...
   if (empty($msg)) { //non ci sono errori
      ...
      //header("Location: invsta_docacq.php");exit;
	  ?>
	  <div id="iframeModal" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999;">
    
			<div style="position:relative; width:90%; height:90%; margin:2% auto; background:#fff; padding:10px; box-shadow:0 0 10px #000;">
				
				<button onclick="chiudiIframe()" 
				style="position:absolute; top:5px; right:5px; font-size:18px; cursor:pointer;">
				✖
				</button>

				<iframe id="docFrame" src="invsta_docacq.php" width="100%" height="100%" style="border:none;"></iframe>
			
			</div>
		</div>
		<script>
		function chiudiIframe() {
...
echo "<div align=\"center\" class=\"FacetFormHeaderFont\">Ristampa documenti gi&agrave; ricevuti " . $script_transl[1];
