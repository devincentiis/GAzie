...
$rooms_table = "{$gTables['room']} LEFT JOIN {$gTables['ward']} ON {$gTables['room']}.id_ward = {$gTables['ward']}.id_ward";

// campi ammissibili per la ricerca
$search_fields = [
    'id_room' => "id_room = %d",
    'id_ward' => "id_ward = %d",
    'roomname'=> "bename LIKE '%%%s%%'"
  ];
...
$(function() {
  $( "#dialog" ).dialog({
	...
	$('.dialog_delete').click(function() {
		$("p#idcodice").html($(this).attr("id_room"));
		$("p#iddescri").html($(this).attr("nome"));
		var id = $(this).attr('id_room');
          ...
          click:function() {
            $(this).dialog("close");
				...
				delete:{
					...
					$.ajax({
						data: {'type':'room',ref:id},
						type: 'POST',
						url: './delete.php',
						success: function(output){
		          //alert(output);
							window.location.replace("./report_rooms.php");
						}
					});
				}}
			}
...
<div class="text-center row">
  ...
  <a class="btn btn-default" href="./admin_room.php"> Nuova stanza </a>
  </div>
...
// colonna edit
          echo '<td class="text-center"><a class="btn btn-xs btn-edit" href="./admin_room.php?id_room='.$r['id_room'].'" title="Modifica nome letto"><i class="glyphicon glyphicon-edit"></i></a>';
          echo "</td>";
// colonna stanza
          echo '<td class="text-center">' . $r['roomname']. " </td>";
...
// colonna reparto
          echo '<td>';
          echo '<a class="btn btn-sm btn-default" target="_blank" href="./admin_ward.php?id_ward=' .$r['id_ward']. '" title="Modifica nome reparto">' . $r['wardname'] . "</a>";
          echo '</td>';
// colonna stanze/letti
          echo '<td>';
          // riprendo il contenuto del reparto (stanze/letti)
          $rs_sl = gaz_dbi_dyn_query("*",$gTables['bed'], $gTables['bed'].".id_room = ".$r['id_room'],$gTables['bed'].".id_bed");
          while ($sl = gaz_dbi_fetch_array($rs_sl)) {
            echo '<a class="btn btn-xs btn-default" href="./admin_bed.php?id_bed='.$sl['id_bed'].'">'.$sl['bedname'].'</a><br/>';
            $con=true;
          }
          echo '</td>';
// colonna elimina
          ...
          if ($con){
            echo 'btn-default" disabled title="Letto non eliminabile perché ci sono letti"';
          } else {
            echo 'btn-elimina dialog_delete" id_room="'. $r['id_room'].'" nome="'.$r['roomname'].' '.$r['wardname'].'" ';
          }
