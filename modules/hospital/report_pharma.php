				...
				delete:{
					...
					$.ajax({
						...
						success: function(output){
		                    //alert(output);
							window.location.replace("./report_pharma.php");
						}
					});
				}}
			}
...
<form method="GET" class="clean_get">
  <div class="framePdf panel panel-success" style="display: none; position: absolute;">
	...
	<tr>
		...
		<td class="FacetFieldCaptionTD">
      <input type="text" name="codfor" placeholder="Fornitore"  class="input-sm form-control" value="<?php echo (isset($codfor))? htmlentities($codfor, ENT_QUOTES) : ""; ?>" maxlength="32">
    </td>
		<td class="FacetFieldCaptionTD">
...
while ($r = gaz_dbi_fetch_array($result)) {
  ...
  echo "<tr>\n";
  echo '<td>
  <a class="btn btn-xs btn-'.$class.'" href="../hospital/admin_pharma.php?codice='.$r['codice'].'" ><i class="glyphicon glyphicon-edit"></i> '.$r['codice'].'</a>';
  if ( $r["good_or_service"] == 2 ) {
    echo '<a class="btn btn-xs btn-warning" href="../hospital/admin_artico_compost.php?Update&codice='.$r['codice'].'" title="Modifica la composizione"><i class="glyphicon glyphicon-list"></i></a>';
  ...
  echo "</td>\n";
  echo '<td class="text-center">'.$r['descat'];
  echo "</td>\n";
  // colonna codice fornitore
	echo '<td class="text-center">'.$r['codice_fornitore'].'</td>';
	echo "</td>\n";
